...
func ComputeFlags(cmd *cobra.Command, options *QodanaOptions) error {
	...
	flags.BoolVar(&options.SendBitBucketInsights, "bitbucket-insights", isBitBucket(), "Send the results BitBucket Code Insights, no additional configuration required if ran in BitBucket Pipelines")
